...
class WP_FCE_Model_Ipn
{
    ...
    /**
     * Get the product mapping for the external product ID
     *
     * @return array|null The product mapping array with keys 'id', 'space_ids' and 'course_ids' if found, null otherwise
     */
    public function get_product_mapping(): array|null
    {
        $mapping = $this->product_helper->get_product_mapping_by_external_product_id($this->external_product_id);
        if ($mapping === null) {
            return null;
        }
        //remove external_product_id because it is not needed
        unset($mapping['external_product_id']);
        return $mapping;
    }
}
